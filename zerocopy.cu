...
int main (int argc, char *argv[]) {
    remove( "log_zerocopy.txt" );
    std::string outfile = "log_zerocopy.txt";
    ...
    if (cp > 0) {
        ...
        cudaHostAlloc((void**) &A_d, bytes, cudaHostAllocMapped);
        cudaHostAlloc((void**) &B_d, bytes, cudaHostAllocMapped);
        cudaHostAlloc((void**) &C_d, bytes, cudaHostAllocMapped);
